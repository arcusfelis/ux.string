...
-import(ux.string).
-include("config.hrl").
-define(TRDATA, ?UNIDATA_DIRECTORY ++ "DerivedNormalizationProps.txt"). 
gen() ->
	{ok, InFd} = file:open(?TRDATA, [read]),
	{ok, NfcQcOutFd}  = file:open(?UX_STRING_INC ++ "nfc_qc.hrl",  [write]),
	{ok, NfdQcOutFd}  = file:open(?UX_STRING_INC ++ "nfd_qc.hrl",  [write]),
	{ok, NfkcQcOutFd} = file:open(?UX_STRING_INC ++ "nfkc_qc.hrl", [write]),
	{ok, NfkdQcOutFd} = file:open(?UX_STRING_INC ++ "nfkd_qc.hrl", [write]),
